...
#define WIFI_SSID   "Kys"
#define WIFI_PASSWD "coolstorybro112"
...
void drawFrame1(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
  ...
  if (digitalRead(AS312_PIN)== 1){
    Serial.println("Triggered");
    long d_time = millis();
    while (millis() - d_time < 6000){
      if (digitalRead(AS312_PIN)==0){
        Serial.println("False Alarm");
        goto bob;
      }
    }
    if (digitalRead(AS312_PIN)== 1){
      Serial.println("Defense Activated");
      delay(3000);
      //Shut blind function
      }
    }
